    ...
    if (result.success) {
      ...
      if (result.errors > 0) {
        swal.fire({
          title: "Error!",
          text:
            "Could not enroll for all of your favorites. Enrolled for " +
            String(result.amountOfFavorites - result.errors) +
            " talks and failed for " +
            String(result.errors) +
            " talks. Did you already enroll for some talks?",
          type: "error",
        });
      } else {
        swal.fire({
...
function talkColumnClick(talk) {
  ...
  if (talk.speaker) {
    ...
    var showFunction = function (title, body, buttonText, buttonClick) {
      $("#ModalTitle").fadeOut(140, function () {
        $(this).html(title).fadeIn(140);
      });
